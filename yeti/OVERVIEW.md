...
internal/views/                 Page builders and event handlers (one file per page)
        ...
        │                       ├── internal/views/bundleview/    │ bundle presentation/action state,
        │                       ├── internal/views/trustmsg/      │ row bookkeeping, expander status,
        │                       ├── internal/views/rowset/        │ Flatpak and feature update-status
        │                       ├── internal/views/flatpakstatus/ │ text and decisions, all unit-tested
...
The updates page tracks counts from bootc, Flatpak, and Homebrew separately (`bootcUpdateCount`, `flatpakUpdateCount`, `brewUpdateCount` fields on `UserHome`) using a `sync.Mutex`. `bootcUpdateCount` is 1 when `bootc.GetStatus()` reports a staged deployment, 0 otherwise — it is not a count of available updates, just a boolean folded into the badge total. The total is pushed to the window's sidebar badge via `ToastAdder.SetUpdateBadge()`. A successful live per-package Homebrew upgrade immediately removes its tracked outdated row and decrements `brewUpdateCount`, then re-queries `ListOutdated`; a successful top-level `brew update` keeps its button busy through the same re-query. Refresh requests receive an increasing generation from `actionstate.RefreshGate`; only the newest request may apply its result, so an older, slower query cannot overwrite newer metadata. Every completion callback still runs when superseded so its action is restored. Only a successful current query replaces rows/count. A command or refresh failure preserves the last known count (and a refresh failure preserves the current rows), while dry-run previews restore their controls without changing either.
...
- **CI mirror**: `make ci` runs every gate `.github/workflows/test.yml` runs, in CI's fail-fast order — go.mod tidy check, `go vet`, gofmt check, `golangci-lint`, unit tests (`./internal/...` under `-run "^Test[^I]" -skip "Integration"`), the race detector, and the build. Its build step reproduces CI's `linux/amd64` + `linux/arm64` matrix into `build/ci-linux-<arch>/` before rebuilding natively, so a compile failure on the non-host architecture cannot pass locally. Green locally means green in CI; the mill's deep gate (`.mill.toml`) calls this exact target.
