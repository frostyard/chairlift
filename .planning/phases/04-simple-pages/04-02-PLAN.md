...
7. `FetchNBCStatus(ctx context.Context) (*nbc.StatusOutput, error)` function:
   - Simple wrapper calling `nbc.GetStatus(ctx)`
   - Returns the same `*nbc.StatusOutput` type that `nbc.GetStatus()` returns
